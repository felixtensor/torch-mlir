...
void mlir::torch::onnx_c::populateComMicrosoftDomain(
  ...
  patterns.onOp(
      ...
      [](OpBinder binder, ConversionPatternRewriter &rewriter) {
        Location loc = binder.getLoc();
        SmallVector<Type> resultTypes;
        Value input, skip, gamma, beta, bias;
        float epsilon;
        if (binder.tensorOperandAtIndex(input, 0) ||
        ...
        if (numOperands > 4) {
          if (binder.tensorOperandAtIndex(bias, 4))
            return failure();
        }
        ...
        // Add bias if present
        if (bias) {
          addResult = Torch::AtenAddTensorOp::create(
              rewriter, loc, addResult.getType(), addResult, bias,
              Torch::ConstantIntOp::create(rewriter, loc,
                                           rewriter.getI64IntegerAttr(1)));
        }
